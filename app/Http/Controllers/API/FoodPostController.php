...
class FoodPostController extends Controller
{
    public function all(Request $request)
    {
        ...
        if($id_user)
        {
            $foodPost = FoodPost::query()->where('id_user', $id_user)->join('users', 'food_posts.id_user', '=' , 'users.id')->where('users.city', '=', $location)->orderBy('created_at', 'desc')->select('food_posts.*', 'users.city')->get();
        ...
        $foodPost = FoodPost::with(['user'])->join('users', 'food_posts.id_user', '=' , 'users.id')->where('users.city', '=', $location)->where('is_verified', true)->orderBy('created_at', 'desc')->select('food_posts.*', 'users.city')->get();

        if ($food_name)
        {
            $foodPost = FoodPost::with(['user'])->where('food_name', 'like', '%' . $food_name . '%')->join('users', 'food_posts.id_user', '=' , 'users.id')->where('users.city', '=', $location)->orderBy('created_at', 'desc')->select('food_posts.*', 'users.city')->get();
        }

        if ($category)
        {
            $foodPost = FoodPost::with(['user'])->where('category', 'like', '%' . $category . '%')->join('users', 'food_posts.id_user', '=' , 'users.id')->where('users.city', '=', $location)->orderBy('created_at', 'desc')->select('food_posts.*', 'users.city')->get();
        }
